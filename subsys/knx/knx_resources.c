...
static inline bool knx_m_value_is(const char *value, size_t value_len, const char *lit)
{
	...
	return value_len == lit_len && memcmp(value, lit, value_len) == 0;
}
...
void knx_get_dp(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
	...
	if (oc_is_redirected_request_from(request) == 1) {
		LOG_DBG("redirected request %.*s", (int)request->uri_path_len, request->uri_path);
	}
	...
	if (oc_query_value_exists(request, "m") != -1) {
		...
		while (oc_iterate_query_get_values(request, "m", &m_value, &m_value_len)) {
			if (m_value_len == 1 && m_value[0] == '*') {
				wildcard = true;
				break;
			}
		}
		...
		oc_init_query_iterator();
		while (oc_iterate_query_get_values(request, "m", &m_value, &m_value_len)) {
			if (knx_m_value_is(m_value, m_value_len, "id") || wildcard) {
			...
			if (knx_m_value_is(m_value, m_value_len, "value") || wildcard) {
				switch (dp->value.kind) {
				case KNX_DPT_BOOL:
					oc_rep_text_set_boolean(root, value,
			...
			if (knx_m_value_is(m_value, m_value_len, "rt") || wildcard) {
			...
			if (knx_m_value_is(m_value, m_value_len, "if") || wildcard) {
				add_all_interface_short_urns_for_a_resource(request->resource);
				error_state = false;
			}
			if (knx_m_value_is(m_value, m_value_len, "dpt") || wildcard) {
				oc_rep_text_set_text_string(root, dpt,
							    oc_string(request->resource->dpt));
				error_state = false;
			}
			if (knx_m_value_is(m_value, m_value_len, "ga") || wildcard) {
				const int index =
					oc_core_find_first_group_object_table_index_from_href(
						oc_string(request->resource->uri));
				if (index > -1) {
			...
			if (knx_m_value_is(m_value, m_value_len, "href") || wildcard) {
				oc_rep_text_set_text_string(root, href,
							    oc_string(request->resource->uri));
				error_state = false;
			}

			// Wildcard emits every field once; don't loop over the remaining values.
			if (wildcard) {
				break;
			}
		}
	...
	if (g_err != CborNoError) {
		LOG_ERR("GET %s failed: CBOR encoding error %d", oc_string(request->resource->uri),
			g_err);
		error_state = true;
	}
		...
		if (rep->iname == 1) {
			if (!is_input_datapoint) {
				...
				oc_prepare_no_format_response_no_payload(
					request, OC_STATUS_METHOD_NOT_ALLOWED);
				return;
			}
			...
			case KNX_DPT_BOOL:
				if (rep->type != OC_REP_BOOL) {
					LOG_ERR("PUT %s rejected: expected boolean payload, got "
						"type %d",
